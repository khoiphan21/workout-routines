...
- **CI:** On pull requests to `main`, workflow [`.github/workflows/hevy-sync-khoiphan21.yml`](.github/workflows/hevy-sync-khoiphan21.yml) validates all bundles, refreshes `libs/hevy/cache/exercise-templates.json`, then pushes to Hevy (same-repo PRs only) with rate-limit retries and throttling. Commits updated `hevy/` JSON back to the PR branch. Requires GitHub secret `HEVY_API_KEY_KHOIPHAN21`.
...
- **List duplicate exercises:** `npm run hevy:list-duplicates` (add `--fetch` for live data) — prints **DELETE** ids only; `--verbose` shows KEEP per title. Reports missing keepers that need `hevy:push` to recreate (new id; old UUID cannot be restored).
